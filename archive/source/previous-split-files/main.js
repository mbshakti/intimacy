...
function experiencePath(p) {
    if (!p || p.startsWith('/') || p.startsWith('http')) return p;
    return EXPERIENCE_BASE + p;
}
    ...
    async function renderDesktopLayout() {
        ...
        const embedPath = experiencePath;
        try {
        ...
        let manifest = {};
        try {
            const manifestUrl = experiencePath('desktop-labels.json');
...
const _clickPool = Array.from({ length: 6 }, () => {
    const a = new Audio('/experience/audio/key-click.wav');
    a.volume = 0.4;
    return a;
});
let _clickIdx = 0;
...
const _pingPool = Array.from({ length: 4 }, () => {
    const a = new Audio('/experience/audio/ping.wav');
    a.volume = 0.5;
...
function setMsgCardBare(isBare) {
    const win = document.getElementById('msgCardWindow');
    win.classList.toggle('window', !isBare);
    win.style.background = isBare ? 'transparent' : '';
    win.style.border = isBare ? 'none' : '';
    win.querySelectorAll('.window-titlebar, .window-path-bar').forEach(el => {
        el.style.display = isBare ? 'none' : '';
    });
}
...
function toggleBotRequest(idx) {
    ...
    panel.scrollTop = 0;
    panel.style.border = messages.length === 1 ? 'none' : '';
