...
const priestData = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'character-data/judgemental_priest.json'), 'utf8')
);
...
app.get('/api/bot-requests', (req, res) => {
  const baseDir = path.join(__dirname, 'bot-requests', 'img');
...
app.get('/api/avatars', (req, res) => {
  const baseDir = path.join(__dirname, 'img', 'discord avatars');
  const exts = new Set(['.png', '.jpg', '.jpeg', '.webp', '.gif']);
  try {
...
app.get('/api/found-images', (req, res) => {
  const baseDir = path.join(__dirname, 'img', 'found images');
  const exts = new Set(['.png', '.jpg', '.jpeg', '.webp', '.gif']);
  try {
    const files = fs.readdirSync(baseDir)
      .filter(f => !f.startsWith('.') && exts.has(path.extname(f).toLowerCase()))
